import {CustomerReviewsPage} from "@/views/admin/customers/pages/customer-reviews-page/CustomerReviewsPage.tsx";
import {
    CustomerReviewsPageContent
} from "@/views/admin/customers/pages/customer-reviews-page/CustomerReviewsPageContent.tsx";
import {
    CustomerReviewsPageHeader
} from "@/views/admin/customers/pages/customer-reviews-page/headers/CustomerReviewsPageHeader.tsx";
import {
    CustomerReviewsPageBreadcrumbs
} from "@/views/admin/customers/pages/customer-reviews-page/headers/CustomerReviewsPageBreadcrumbs.tsx";

export {
    CustomerReviewsPage,
    CustomerReviewsPageContent,
    CustomerReviewsPageHeader,
    CustomerReviewsPageBreadcrumbs,
}