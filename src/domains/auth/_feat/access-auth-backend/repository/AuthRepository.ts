...
export function registerUser(data: AuthRegisterForm): Promise<FetchRequestReturns> {
    const url = buildURL({baseURL: AuthBaseURL, path: "/register"});
    return useFetchAPI({url, method: "POST", data});
}

/** Authenticates a user with the provided login credentials. */
export function loginUser(data: AuthLoginFormData): Promise<FetchRequestReturns> {
    const url = buildURL({baseURL: AuthBaseURL, path: "/login"});
    return useFetchAPI({url, method: "POST", data});
}

/** Terminates the current user session. */
export function logoutUser(): Promise<FetchRequestReturns> {
